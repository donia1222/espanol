...
Es gilt Schweizer Recht. Gerichtsstand ist Buchs SG.`);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      navRef.current?.classList.toggle("scrolled", y > window.innerHeight - 100);
      const scrollingDown = y > lastScrollY.current;
      lastScrollY.current = y;
      // Scroll down + past hero = show scroll-to-top; scroll up anywhere = WhatsApp
  ...
  return (
    <>
      ...
      {/* Floating action button: WhatsApp or Scroll-to-top */}
      {showScrollTop ? (
        <button
          className="c-whatsapp-float c-fab-top"
          aria-label="Scroll to top"
          onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
        >
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="18 15 12 9 6 15"/></svg>
        </button>
      ) : (
        <a
